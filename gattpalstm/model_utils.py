...
from gattpalstm.model import GATTPALSTM


def create_data_generator(para):
    if para.mts:
        from gattpalstm.data_generator import TimeSeriesDataGenerator
...
def create_graph(para):
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('model', initializer=initializer):
            model = GATTPALSTM(para, data_generator)
...
def save_model(para, sess, model, extra=""):
    [global_step] = sess.run([model.global_step])
    checkpoint_path = os.path.join(para.model_dir, "model" + str(para.num_var) + extra + ".ckpt")
