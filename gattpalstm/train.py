...
from gattpalstm.setup import config_setup
from gattpalstm.model_utils import save_model, create_valid_graph, load_weights
...
def train(para, sess, model, train_data_generator):
    ...
    with tf.Session(config=config_setup(), graph=valid_graph) as valid_sess:
        ...
        for epoch in range(1, para.num_epochs + 1):
            ...
            # validation || It is rained, no need to load again
            ...
            if para.mts:
                ...
                if valid_loss < best_val_loss and para.num_epochs > 30:
                    best_val_loss = valid_loss
                    best_val_rse = valid_rse
                    best_val_corr = valid_corr

                    logging.info(
                        "best: validation loss: %.5f, validation rse: %.5f, validation corr: %.5f",
                        best_val_loss, best_val_rse, best_val_corr)
                    save_model(para, sess, model, "best")
            else:
